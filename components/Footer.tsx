      ...
      <div className="pb-14 pt-28">
        <div className="flex flex-col items-center justify-center gap-10 pb-10 lg:flex-row lg:items-stretch lg:justify-between lg:pb-32">
          ...
          <div className="space-y-5">
            <h6 className="bg-gradient-to-r from-yellow-400 to-yellow-200 bg-clip-text text-center text-lg font-bold text-transparent lg:text-left">
              Explore
            </h6>
            <ul className="flex max-w-sm flex-row flex-wrap justify-center gap-4 font-medium text-gray-400 lg:max-w-full lg:flex-col">
              {footerLinks[0].map((item, i) => (
                <Link key={i} href={item.link} className="hover:text-yellow-400 transition-colors">
                  {item.title}
                </Link>
              ))}
            </ul>
            ...
            <div className="flex h-full flex-col justify-between">
              <div className="flex flex-col justify-center gap-5 pt-8 lg:pt-0">
                ...
                <form className="flex w-full max-w-sm items-center space-x-2">
                  <input
                    ...
                    className="bg-gray-800 border-gray-700 text-white rounded-md p-4"
                  />
                  <button type="submit" className="bg-yellow-500 text-black hover:bg-yellow-400 rounded-lg p-4">
                    Subscribe
                  </button>
                </form>
              ...
              <div className="flex justify-center gap-5 pt-10 text-2xl text-gray-400 md:justify-normal lg:pt-0">
                {socialLinks.map((item, i) => (
                  <Link key={i} href={item.link} target="_blank" rel="noopener noreferrer" className="hover:text-yellow-400 transition-colors">
                    <item.icon />
                  </Link>
                ))}
              </div>
